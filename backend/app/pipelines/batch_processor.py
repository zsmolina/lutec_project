...
class BatchProcessor:
    def __init__(
        ...
        self.format = invoice_format
        self.settings = settings or get_settings()
        self.extractor = get_extractor(invoice_format)
    ...
    def _process_entries(
        ...
        exporter = ExcelExporter(excel_output, self.extractor.excel_column_config)
        outcomes: list[InvoiceProcessOutcome] = []
        succeeded = 0
        failed = 0

        logger.info("Iniciando lote %s — %d PDF(s)", self.format.value, total)
        ...
        for index, entry in enumerate(entries, start=1):
            ...
            if on_progress:
                on_progress(index - 1, total, entry.name, succeeded, failed)
        ...
        logger.info(
            "Lote %s finalizado: %d/%d exitosos → %s",
            self.format.value,
            succeeded,
            total,
            excel_output,
    ...
    def _process_single_pdf(
        ...
        try:
            data = self.extractor.extract(entry.data, self._client, self.settings)
